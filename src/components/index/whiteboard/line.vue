...
<script>
  ...
  export default {
    ...
    mounted:function(){
      setTimeout(this.gzzbqkFunction(),5000);
      setTimeout(this.gzzbqktwoFunction(),5000)
    },
    methods:{
      gzzbqkFunction(){
        var myChart = Vue.prototype.echarts.init(document.getElementById('lineszsrbh'));
        myChart.setOption({
          title: {
            text: '税种收入变化',
            textStyle: {
      ...
      gzzbqktwoFunction(){
        var myChart = Vue.prototype.echarts.init(document.getElementById('sztbbh'));
        myChart.setOption({
          title: {
            text: '税种同比变化',
            textStyle: {
